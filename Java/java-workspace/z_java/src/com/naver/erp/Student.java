...
**************************************
    ...
    ================================================================================
        ...
        ---------------------------------
            시험문제 타겟!
            --------------------------------------------------------------------
            <의미1> static 이 붙은 메소드는 객체생성없이 아래 처럼 호출이 가능하다. 
            --------------------------------------------------------------------
                ---------------------------------
                클래스명.메소드명(~)
                ---------------------------------
        ...
        ---------------------------------
            ...
            자식이 오버라이딩(=overriding)하는 것을 금지하는 이유는 이것 말고는 사용하지 말라는 의미이다.

            --------------------------------------------------------------------
            오버라이딩(=overriding)
            --------------------------------------------------------------------
                부모 클래스의 메소드를 자식 클래스 안에서 재 정의하는것을 오버라이딩 이라고 한다.  
                
                오버라이딩 시 메소드 앞에 @Override 라는 어노테이션(=annotation) 를 붙이면 좋다.

                ★ 오버라이딩 규칙
                    부모메소드명과 동일하다.  
                    리턴형이 동일해야 한다.  
                    매개변수의 개수와 자료형이 일치해야 한다.  
                    접근지정자는 부모것 보다 같거나 커야된다.  
            --------------------------------------------------------------------
            오버로딩(=overloading)
            --------------------------------------------------------------------
                동일한 클래스 내부에 같은 이름의 메소드를 2개 이상 정의 하는것을 말한다.  

                ★ 오버로딩 규칙
                    메소드 이름은 같다.
                    메소드의 매개변수의 개수가 다르거나 매개변수 개수가 같으면 자료형이 달라야 한다.
                -------------------------------------------
                <예>
                -------------------------------------------
                ...
                -------------------------------------------
            --------------------------------------------------------------------
            <참고> 자바에서 어노테이션(=annotation)     (입) => 입사 시험문제
            --------------------------------------------------------------------
                클래스명 앞에, 속성변수 앞에, 메소드 앞에, 메소드의 매개변수 앞에 붙어서
                특정 기능을 부여하는 자바의 표기법을 말한다.  
                어노테이션의 표기방식은       @영문        인데, 이때 영문은 반드시 대문자로 시작한다.   
                Spring framework 에서 엄청 즐겨쓴다.  
                이거 없으면 Spring framework 에서 날 코딩이 엄청 많아진다.  
           
        ---------------------------------
        abstract             => 시험문제 타겟!
        ---------------------------------
            -----------------------------------------------------------
            메소드에 블록`{}`(=바디) 이 없는 경우 붙인다.
            메소드에 블록`{}`(=바디) 이 없는 경우 이 메소드를 소유한 클래스를 추상클래스라고 부른다.
            -----------------------------------------------------------
            추상 = 구체적이지 않음 = 애매모호함.
            -----------------------------------------------------------
            블록(=바디={}) 가 없는 메소드, 즉, abstract 가 붙은 메소드를 가진 클래스의 특징
            -----------------------------------------------------------
                클래스 앞에도 abstract 가 붙는다.  public abstract class Xxx{ public abstract void aaa(); }
                객체화가 불가능하다. 
                    객체화의 목적은 메소드 호출인데 블록(=바디={}) 가 없는 메소드는 호출이 불가능하므로
                    객체화도 될 수 없다.
                    그러므로, 자식 클래스가 상속받아서 블록(=바디={}) 가 없는 메소드를 재정의하라는 뜻이 강한 것이다.  
                블록(=바디={}) 가 없는 메소드를 가지고 있지 않은데도 객체화되지 않는 경우가 있다. 
                이 경우 조상 중에 블록(=바디={}) 가 없는 메소드를 가지고 있는데, 조상 클래스와 후손 클래스 사이의 
                그 누구도 블록(=바디={}) 가 없는 메소드를 재정의 하지 않은 경우이다.  

            -----------------------------------------------------------
            <주의> 실행구문이 없는것과 블록(=바디={})이 없는 경우는 다르다.
            -----------------------------------------------------------
                   실행구문이 없는 예                    => public void xxx(){}
                   블록(=바디={})이 없는 경우 없는 예     => public abstract void xxx();
            -----------------------------------------------------------
            <예> public int plus();
